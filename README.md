...
Keep using `ralph` for larger migrations, broad refactors, multi-package changes, and spec-to-reviewed-change work where you want Atomic to plan the approach, delegate implementation through sub-agents, simplify, review, and iterate. Add `create_pr=true` only when you want the final pull-request stage and report.
...
| `ralph`                  | Heavier plan → orchestrate → simplify → discover → review loop for larger migrations, broad refactors, multi-package changes, and spec-to-reviewed-change work. It writes RFC-style specs, delegates implementation through sub-agents, iterates on reviewer feedback, and only prepares a pull-request report when `create_pr=true`. | `/workflow ralph prompt="Plan, implement, and review specs/2026-03-rate-limit.md" create_pr=true`                                                                                             |
