...
describe("createStageContext — reasoning suffix retry behavior", () => {
	async function exerciseRetryWithReasoning(invoke: (ctx: InternalStageContext) => Promise<string>): Promise<{
	...
	test("direct task retry applies suffixed reasoning and records attempt metadata", async () => {
		...
		assert.deepEqual(result.thinkingLevels, ["anthropic/primary:high", "openai/fallback:low"]);
		assert.deepEqual(
		...
		// The fallback model — not the failed primary — is surfaced to graph UIs.
	...
	test("legacy thinkingLevel applies when candidates have no suffix", async () => {
		...
		assert.deepEqual(thinkingLevels, ["anthropic/primary:medium", "openai/fallback:medium"]);
		assert.deepEqual(
