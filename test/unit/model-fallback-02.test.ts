...
describe("context-window authoring token", () => {
    ...
    function copilotOpus(options: {
        ...
        return {
            ...
            model: {
                ...
                ...(options.contextWindowOptions !== undefined
                    ? { contextWindowOptions: options.contextWindowOptions }
                    : {}),
    ...
    test("only the tokened candidate carries a context window; siblings are untouched", () => {
        const catalog: readonly WorkflowModelInfo[] = [
            {
                provider: "anthropic",
                id: "claude-fable-5",
                fullId: "anthropic/claude-fable-5",
            },
            ...tieredOpus,
        ];
        ...
        const primary = candidates.find(
            (c) => c.id === "anthropic/claude-fable-5",
        );
        const opus = candidates.find(
            (c) => c.id === "github-copilot/claude-opus-4.8",
        );
