...
function makeCompletedPromptArchiveView(message: string, response: string): StageChatView {
...
describe("StageChatView", () => {
  ...
  test("forwards question keys (incl. ESC) to the custom UI but keeps scroll keys for the transcript", async () => {
    ...
    const broker = new StageUiBroker(store);
    const { handle } = makeHandle(undefined, [
      { role: "assistant", content: [{ type: "text", text: "HISTORY-LINE" }] },
    ] as unknown as AgentSession["messages"]);
    const view = new StageChatView({
      store,
      graphTheme: deriveGraphTheme({}),
  ...
  test("fuzz: random input never crashes the stage chat (custom UI mounted and idle)", async () => {
    ...
    const alphabet = [
      ...
      "\x1b[<64;1;1M", "\x1b[<65;10;10M", "\x1b[M   ", "\x1bOH", "\x1bOF",
      "\x01", "\x05", "\x0b", "\x15", "\x17", "\u00e4", "\ud83d\ude80",
      "\x1b[200~paste\x1b[201~", "\x00", "\x1b[<0;5;5m",
    ];
    ...
    // never resolves and the alphabet omits ctrl+c/ctrl+d, so it stays mounted.
    {
      ...
      const broker = new StageUiBroker(store);
      const { handle } = makeHandle(undefined, [
        { role: "assistant", content: [{ type: "text", text: "FUZZ-HISTORY" }] },
      ] as unknown as AgentSession["messages"]);
      const view = new StageChatView({
        store,
        graphTheme: deriveGraphTheme({}),
    ...
    // Phase B: idle live stage (composer path) — include teardown keys too.
    {
      ...
      setupRun(store, "run-1", "stage-a");
      const { handle } = makeHandle(undefined, [
        { role: "assistant", content: [{ type: "text", text: "FUZZ-IDLE" }] },
      ] as unknown as AgentSession["messages"]);
      const view = new StageChatView({
        store,
        graphTheme: deriveGraphTheme({}),
  ...
  test("propagates focus to the nested pi editor for hardware cursor placement", () => {
    ...
    view.focused = false;
    assert.doesNotMatch(view.render(96).join("\n"), new RegExp(CURSOR_MARKER));
    view.dispose();
  });
  ...
  test("renders pi-style spacing between a full transcript and the streaming loader", () => {
    ...
    assert.ok(workingIndex > 1, "expected working spinner after transcript");
    const previousContent = lines.slice(0, workingIndex).findLast((line) => line.trim() !== "");
  ...
  test("attached live sessions render the usage ribbon above the chatbox and only the coding-agent footer below it", () => {
    ...
    assert.doesNotMatch(rendered, /pageup\/pagedown|ctrl\+d|follow-up|steer/);
    view.dispose();
  });
  ...
  test("renders toolcall_end call contents when workflow event snapshots are stale", () => {
    ...
    emit({
      ...
      message: {
        role: "assistant",
        content: [{ type: "toolCall", id: "t-stale", name: "bash", arguments: {} }],
      },
      assistantMessageEvent: {
        type: "toolcall_end",
        contentIndex: 0,
        toolCall: { type: "toolCall", id: "t-stale", name: "bash", arguments: { command: "echo from-workflow" } },
      },
    } as unknown as AgentSessionEvent);
    emit({ type: "tool_execution_start", toolCallId: "t-stale", toolName: "bash" } as unknown as AgentSessionEvent);
