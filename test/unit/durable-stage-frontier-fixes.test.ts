...
test("parallel replay uses fail-fast scope parents instead of flattening fanout", () => {
	...
	const setup: DurableCompletedStageCheckpoint = {
		kind: "stage",
		workflowId: WORKFLOW_ID,
		checkpointId: "stage:setup:1",
		name: "setup",
		replayKey: "stage:setup:1",
		output: "ready",
		completedAt: 1100,
		result: "ready",
	};
	...
	for (const name of ["review-a", "review-b"]) {
		const cp: DurableCompletedStageCheckpoint = {
			kind: "stage",
			workflowId: WORKFLOW_ID,
			checkpointId: `task:stage:task:${name}:1`,
			name,
			replayKey: `stage:task:${name}:1`,
			output: { name, stageName: name, text: `${name} done` },
			completedAt: 1200,
			result: `${name} done`,
			topology:
				name === "review-a"
					...
					: {
							version: 1,
							stageId: cpStageId(name),
							parentIds: ["missing-source-parent"],
							run: { runId: WORKFLOW_ID, runName: "wf" },
						},
		};
...
test("cached replay hydrates persisted stage timing, result, session, and model + thinking metadata", () => {
	...
	const cp: DurableCompletedStageCheckpoint = {
		kind: "stage",
		workflowId: WORKFLOW_ID,
		checkpointId: "stage:hydrate:1",
		name: "hydrate",
		replayKey: "stage:hydrate:1",
