...
and boots the CI environment in the background while you work:
    ...
    blacksmith testbox warmup test-testbox-windows.yml
    # → tbx_01jkz5b3t9...
...
1. Warm up immediately when you receive a coding task:
   `blacksmith testbox warmup test-testbox-windows.yml` → save the ID
...
If you need a longer session, increase the timeout at warmup time:
    ...
    blacksmith testbox warmup test-testbox-windows.yml --idle-timeout 60

## With options

    blacksmith testbox warmup test-testbox-windows.yml --ref main
    blacksmith testbox warmup test-testbox-windows.yml --idle-timeout 60
