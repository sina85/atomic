...
- Fixed cross-session `/workflow resume` so exact `--session` restoration cannot let a restored local snapshot shadow the authoritative durable root: an ended snapshot no longer diverts to read-only mode, and a restored `running` snapshot after a hard exit (Ctrl-C/crash before the graceful quit-to-paused flush) is no longer refused as "already running in this session" because active-run detection now consults the real execution lease (`isWorkflowExecutionActive`) rather than the reconstructed store entry, clearing the ghost snapshot before re-dispatch; added per-dispatch heartbeat-backed file leases and PostgreSQL advisory DBOS leases so active roots are hidden/refused across processes and hosts while crashes recover without evicting live suspended owners; a dropped Postgres advisory-lock connection stops reporting ownership and fences (aborts) the local executor so the workflow can be reclaimed without double-dispatch; process-generation identity is derived locale/timezone-independently (Linux `/proc` start-time or `LC_ALL=C`/`TZ=UTC` `lstart`) so two same-host processes never disagree about a live PID, and a confirmed-live same-host PID is never evicted on heartbeat age alone — only a dead or identity-mismatched owner is reclaimed, so a long synchronous stage cannot be double-dispatched; transient heartbeat read/write failures are retried, a completed workflow no longer leaks its heartbeat interval past terminal pruning, and terminal pruning of a completed workflow never deletes an execution lease another live process has since claimed; foreign-host file leases are conservatively pinned to prevent cross-host double-dispatch, since heartbeat age reflects the owner's event loop and cross-host coordination must use the connection-fenced DBOS/PostgreSQL backend; retained ownership through paused metadata until execution stops; a cancellation arriving during the resume startup flush is honored before the run is reported published, so a cancelled resume rolls back to its prior resumable state instead of being recorded as a successful dispatch; a durable flush error during CLI quit no longer skips the remaining shutdown cleanup; resume re-validates the authoritative durable handle after claiming so a workflow completed or pruned by a racing process between the read and the claim is refused (and the just-acquired claim released) rather than resurrected; a resume that fails before dispatch releases this process's own claim, while a failure after dispatch leaves the engine's own lease release intact and never double-releases over a newer rapid-resume owner or rolls its status back; made file-lock handoff atomic and process-generation aware and made stale-lock reclamation compare the reclaimed lock against the exact owner snapshot it validated as abandoned; made `WorkflowFileDurableBackend` reads revalidate against disk (and clear its in-memory execution claim on reset) so an externally pruned completed root cannot be resurrected from a stale cached handle; revalidate stale picker rows and DBOS metadata authoritatively (a targeted remote-ownership refresh clears a stale external-owner marker before refusing a restored-running resume); await durable setup before reporting resume success; encode Windows-illegal workflow-id characters; refuse definition-incompatible resume; and order resumable roots deterministically. Cross-host coordination beyond same-host duplicate-dispatch prevention (foreign-host file-lease reclamation) is intentionally delegated to the connection-fenced DBOS/PostgreSQL backend; `/workflow kill` clears a stuck workflow's durable state.
