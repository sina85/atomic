...
- Changed workflow model validation so provider-qualified Cursor strings and `Model` objects must match the authenticated exact live catalog before stage session creation. Only the literal lowercase `cursor/` prefix and exact lowercase object provider `cursor` activate Cursor discovery; case or whitespace variants and bare (non-qualified) references retain unchanged ordinary non-Cursor pass-through behavior rather than receiving Cursor errors. Provider-qualified text—including `cursor/` for a blank route—selects the first ordered occurrence byte-for-byte; generic trimming, context tokens, reasoning suffixes, and fuzzy/bare lookup never rewrite another string into a Cursor route. A selected Cursor object resolves to the live catalog occurrence (its private per-ID ordinal when valid, else the first occurrence) rather than the caller object, so its executable api, base URL, and current duplicate-occurrence Max/image routing come only from the authenticated catalog. Stale Cursor references fail with reselection guidance without configured/current/provider/model fallback, generic non-Cursor behavior remains unchanged, and builtins no longer hard-code account-specific Cursor routes ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
- Awaited the shared `model_catalog_discover` lifecycle before reading the registry for explicit lowercase `cursor/` strings and exact-lowercase-provider Cursor objects; bare references reserve no Cursor discovery and Atomic retains no static list of historical Cursor IDs. Named `workflow({ action: "run" })` launches now perform the same preflight before creating a Run/store/stage entry; concurrent launches share discovery, cancellation or an unavailable exact route creates no Run and cannot fall through, and non-Cursor launches are not delayed ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
...
- Required workflow `Model` objects to use the exact lowercase `cursor` provider value before entering authenticated Cursor discovery and catalog validation. Case- or whitespace-normalized provider values retain ordinary non-Cursor object pass-through behavior ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
