...
- Fixed compact workflow `Loop:` visibility so medium-width non-loop graph overlays keep the `navigate`/`attach` labels, expanded child workflow graphs get a loop rail, builtin non-positive loop limits show runner defaults instead of `0 remain`, and ordinary numeric stage labels like `oauth-2` are not treated as loop counters unless a counted family repeats.
- Fixed workflow loop rail polish by removing a committed validation artifact, using the graph statusline's already-expanded display stages consistently, accepting generic `loops_completed` result counters and singular builtin loop-limit aliases, and aligning detail-view remaining-loop wording with active vs terminal runs.
- Fixed workflow loop rails to reuse the builtin runners' exported default loop limits instead of duplicating `ralph`, `goal`, and `open-claude-design` fallback values in the TUI summary helper.
...
- Fixed workflow stage chats to leave terminal text selection enabled by default, so users can drag-select and copy workflow prompts, logs, command text, and model responses from attached workflow chats; `ctrl+t` now explicitly toggles temporary mouse-scroll capture for users who prefer mouse/trackpad wheel scrolling. ([#1519](https://github.com/bastani-inc/atomic/issues/1519))
