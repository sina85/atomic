...
- Fixed cross-session `/workflow resume` so exact `--session` restoration cannot let a restored local snapshot shadow the authoritative durable root: an ended snapshot no longer diverts to read-only mode, and a restored `running` snapshot after a hard exit (Ctrl-C/crash before the graceful quit-to-paused flush) is no longer refused as "already running in this session" because active-run detection now consults the real execution lease (`isWorkflowExecutionActive`) rather than the reconstructed store entry, clearing the ghost snapshot before re-dispatch; added per-dispatch heartbeat-backed file leases and PostgreSQL advisory DBOS leases so active roots are hidden/refused across processes and hosts while crashes recover without evicting live suspended owners; a dropped Postgres advisory-lock connection stops reporting ownership so the workflow can be reclaimed; a same-host reused PID whose process-generation identity cannot be confirmed is reclaimed once its heartbeat is stale instead of pinning indefinitely, and transient heartbeat read/write failures are retried; retained ownership through paused metadata until execution stops; made file-lock handoff atomic and process-generation aware and made stale-lock reclamation compare the reclaimed lock against the exact owner snapshot it validated as abandoned; made `WorkflowFileDurableBackend` reads revalidate against disk (and clear its in-memory execution claim on reset) so an externally pruned completed root cannot be resurrected from a stale cached handle; revalidate stale picker rows and DBOS metadata authoritatively; await durable setup before reporting resume success and roll back failures safely; encode Windows-illegal workflow-id characters; refuse definition-incompatible resume; and order resumable roots deterministically.
