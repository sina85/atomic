...
- Fixed background workflow activity flickering the main-chat tail and snapping native terminal scrollback back to the live bottom after wheel-scrolling away ([#1856](https://github.com/bastani-inc/atomic/issues/1856)). The below-editor workflow companion widget no longer keeps a once-per-second elapsed-clock repaint — elapsed labels (`3s`, `complete · 4s ago`) now advance on semantic workflow/stage transitions and when the graph/stage overlay opens, with the only remaining timer being the one-shot recent-ended expiry that removes an ended run's card ~30s after completion. Store mutations that leave the rendered card byte-identical no longer broadcast a host-wide render. In addition, a hidden workflow graph/stage overlay no longer requests host renders on store updates: retained view state is still invalidated so reopening (toggle or `/workflow connect`) explicitly repaints from the current snapshot, while a visible overlay keeps its own live clock, mouse ownership, and scroll anchors. Steady-state background streaming now writes zero terminal bytes for clock-only ticks and produces no full-screen or scrollback clears.
