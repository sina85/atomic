 ...
 */
export function computeLayout(stages: readonly StageSnapshot[], opts: LayoutOpts = {}): LayoutNode[] {
	...
	const bandWidth = (count: number): number => (count > 0 ? count * NODE_W + (count - 1) * colGap : 0);
	const widestBand = (() => {
		let max = 0;
