...
- Fixed workflow stages freezing when their active foreground subagent sent Intercom traffic or blocking `contact_supervisor`. Busy open stages now give the exact foreground owner probe/commit detach handshake first refusal before admitting the message through the stage AgentSession generation boundary; unclaimed traffic and owner loss between probe/commit fall back to normal admission instead of dropping a broker-delivered message. A commit from one parallel child releases aggregate foreground supervision for all active siblings while their processes and eventual results remain owned.
