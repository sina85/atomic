...
- Fixed first-time Cursor OAuth login reporting success while authenticated discovery had failed, been superseded by another credential, or remained pending during shutdown; Atomic now requires a live catalog for the login's credential scope, tracks that discovery through disposal cancellation, and surfaces a redacted error instead of saving an apparently authenticated provider backed only by estimated membership ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
...
- Fixed `/model`, CLI/default resolution, and session restore for authenticated Cursor parameter variants: the picker now receives concise effort-neutral IDs and Cursor-derived mode names, prior synthetic IDs migrate to a unique canonical row, ambiguous aliases are rejected, and exact current IDs continue to take precedence ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
- Fixed CLI and session restoration fabricating selectable `cursor/<unknown-or-ambiguous-id>` models with no Cursor routing metadata; Cursor resolution now requires an exact current ID or one unambiguous compatibility alias ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
- Fixed Cursor CLI references being resolved against the estimated catalog before authenticated discovery; Atomic now asks dynamic providers for catalogs without consuming `session_start`, then the real TUI/RPC binding receives its normal UI-aware lifecycle event. Every case-insensitive Cursor reference—including compatibility-snapshot hits and already-qualified `--model` values under `--provider`—must resolve to an exact authenticated ID or unambiguous alias, effort-bearing aliases preserve their reasoning level, fuzzy/similar rows are never substituted, explicit reasoning/context-window options are reapplied, and invalid deferred context windows use normal concise startup diagnostics ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
- Fixed fixed thinking/context Cursor presets without selectable effort controls failing from inherited `off` reasoning or a route-less concise ID; fixed presets no longer expose synthetic effort controls, and unmarked presets retain their exact advertised route ID instead of receiving a synthetic primary route ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
