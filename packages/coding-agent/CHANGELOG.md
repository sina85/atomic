...
- Added the additive v2 **full-collapse** verbatim compaction format (`details.format: "full-collapse"`, `promptVersion: 4`). New compaction boundaries serialize the entire current context — the prior compacted string plus every context-visible message since it — into one region and collapse it into a single verbatim string, rather than a compacted string plus a replayed structured tail. `preserve_recent` now designates exactly the last N context-visible messages, which are retained byte-identically as protected lines inside that string; there is no user-turn widening (`selectCut`) and no structural protection for roles, tool calls, or call/result pairing beyond those messages. Repeated compaction within one user turn folds the previous string plus everything appended after it into a new boundary.
- Added host validation for both full-collapse planner protocols: isolated requests accept only an ordered, byte-identical subsequence of the canonical source, while supported warm-cache requests accept exact `KEEP` line identifiers. Both retain every protected line, delete at least one useful line, and mechanically reconstruct canonical `(filtered N lines)` markers without ambiguity from duplicate source text.
- Added conservative compaction-call prompt-cache reuse for v2 full-collapse. Atomic captures the actual final provider payload at the normal request's pre-transport hook, preserves its provider-native historical prefix exactly once, and appends only a deterministic compaction suffix. Reuse is limited to mechanically provable simple role/text mappings with matching model, provider, API, base URL, cache routing, and transport; every unsupported shape or insufficient-headroom request falls back to an isolated compaction view without changing durable messages.
- Added API-specific prompt-cache handling before final payload capture: Anthropic uses `cache_control`, public OpenAI GPT-5.6+ Responses/Chat APIs may use `prompt_cache_breakpoint: {"mode":"explicit"}`, and Codex/Azure Responses never receive that unsupported field. Unmarked Responses payloads can use provider-native automatic caching with the preserved `prompt_cache_key`; undefined optional properties follow JSON-wire omission semantics, while non-JSON shapes disable warm capture without changing transport behavior.
- Added normalized warm-compaction cache telemetry on the result and `details.cache` (`cacheReadTokens`, `cacheWriteTokens`, `cacheHit`, `provider`, `model`); isolated fallbacks omit telemetry, and a cache hit is never reported unless the provider reports nonzero cache-read tokens.
...
- Kept the interactive-engine watchdog's early 250 ms blocking signal internal instead of rendering repetitive "has not yielded" warnings during ordinary slow extension/tool loading. One-second unresponsive watchdog diagnostics are likewise kept internal unless they attribute a specific stuck callback (e.g. `tool.execute <name>`): unattributed "unknown callback" heartbeat gaps are ordinary engine event-loop lag while the TUI stays responsive, so they no longer surface as chat errors. Attributed stuck-callback diagnostics still render with Escape/Ctrl+C recovery guidance, and real engine failures (termination, RPC errors) are unaffected.
- Changed session reconstruction to dispatch on the boundary format: v2 full-collapse boundaries rebuild as the compacted string plus only the entries appended after the boundary (the `firstKeptEntryId` self-anchor is ignored), while legacy boundaries keep their exact string-plus-kept-tail behavior. This removes the user-turn-aligned dead end that could report "nothing more was safely deletable" while hundreds of assistant/tool entries remained inside one logical turn; overflow now surfaces that message only when the serialized region is below the minimum size or every eligible line is protected.
...
- Fixed full-collapse overflow recovery near and beyond the model context limit. Atomic now captures the actual preceding provider request before dispatch, reuses that cached prefix exactly once, carries post-prefix context exactly once, and omits an auto-derived latest-user relevance focus already present in that canonical history while retaining explicit caller/settings focus once in the suffix. It composes normal payload transformations with cache breakpoints and falls back to a fit-based isolated compaction view when the prefix cannot leave output headroom. Oversized tool results are bounded only in the compaction request while append-only session bytes remain unchanged; requested output respects provider input/output and total-context limits, and input overflow is diagnosed separately from compaction output exhaustion.
