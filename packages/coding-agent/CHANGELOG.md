...
- Added host validation that accepts the compaction model's returned compacted string only when it is an ordered, byte-identical subsequence of the serialized source that retains every protected line and deletes at least one useful line. Rewrites, reordering, duplicated lines, and dropped protected lines are rejected; the persisted string is always host-reconstructed with canonical `(filtered N lines)` markers.
- Added compaction-call prompt-cache reuse for the v2 full-collapse request. The compaction request now starts with the exact active-request prefix (same model, provider, tools, system prompt, old messages, and cache routing — `sessionId` / `prompt_cache_key`, cache retention, transport) and appends the compaction instruction after the cache breakpoint, so the provider serves the old conversation from its existing prompt cache. No compaction system prompt, numbering, or serialization is placed before the breakpoint, and tools/schema are never changed for compaction.
- Added provider-specific compaction cache breakpoints on top of automatic caching: an explicit Anthropic `cache_control` breakpoint on the final old-conversation block (respecting the four-breakpoint and 20-block limits), and an OpenAI GPT-5.6+ `prompt_cache_breakpoint: {"mode":"explicit"}` on the final old-conversation content block with a compatible automatic-caching fallback for older models. Custom and unsupported providers keep their existing behavior.
- Added normalized compaction cache telemetry on the result and `details.cache` (`cacheReadTokens`, `cacheWriteTokens`, `cacheHit`, `provider`, `model`); a cache hit is never reported unless the provider response usage reports nonzero cache-read tokens.
