...
- Documented and integrated Cursor's GetUsable-authoritative catalog as an ordered sequence with one model occurrence per returned row, including blank/whitespace and duplicate IDs. Raw textual exact lookup uses the first occurrence without trimming, suffix/context parsing, ambiguity rejection, or rewriting; only the literal lowercase `cursor/` prefix identifies provider-qualified Cursor text, and a lowercase `cursor/<id>` with no live route is a terminal reselection failure rather than a fall-through to a same-named custom row. Saved/session/default/object identities likewise require provider exactly lowercase `cursor`; case or whitespace variants and bare (non-qualified) references retain ordinary non-Cursor resolution and cannot activate Cursor discovery, errors, or execution authority, and Atomic keeps no static list of historical Cursor IDs. A workflow-selected Cursor model object resolves to the live catalog occurrence rather than the caller object, so executable routing comes only from the authenticated catalog; a selected later in-memory model object survives workflow validation and catalog refresh with its current occurrence-specific routing. Persisted provider/ID references intentionally restore the first occurrence. The selected flat ID and GetUsable Max state are sent in both request model structures, `RequestedModel.parameters` is empty, and tuple expansion, backend-route synthesis, and route-rewriting reasoning selectors remain removed ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
- Limited the separate best-effort AvailableModels operation to same-account image enrichment that requires every matching exact identity/variant record to report true. Its decoder preserves explicit blank/whitespace identities, ordered duplicate variants, optional flags, and identityless parent rows without fabricating omitted strings. Duplicate or distinct agreeing matches remain image-capable; any false or omitted match is text-only. AvailableModels cannot add, remove, reorder, collapse, or change GetUsable membership, route identity, display/Max data, parameters, or text availability ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
- Upgraded Cursor's token-free account-scoped catalog to schema v3 while preserving the ordered GetUsable row sequence, including blank/whitespace and duplicate IDs, plus optional image flags. Only a fresh same-account snapshot may bridge a temporary discovery failure, while old, stale, AvailableModels-only, and static data cannot become executable ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).

### Fixed

- Preserved Cursor authenticated discovery before explicit resolution, 30-minute TTL refresh, same-account credential rotation, out-of-order refresh fencing, redacted/abortable login gating, disposal cancellation, and strict current-turn user/live-MCP image serialization while removing old automatic migration and fallback paths. Image bytes require canonical non-empty standard base64 after ASCII whitespace stripping; MIME strings are forwarded without allowlist validation, with unknown values using `.img`. Historical image blocks are omitted while text/tool continuity remains ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
...
- Enforced Cursor's host-selected current-account GetUsable catalog through live revocable execution leases: caller credentials are checked against the host-selected scope before cold cache activation or discovery; selected occurrence metadata chooses only a current GetUsable occurrence; and a selected account change unregisters prior rows and joins the new account's cache/GetUsable discovery. Stored and execution resolver results are epoch-fenced before side effects, stale/post-disposal outcomes are inert, and current missing/failed execution credentials revoke rows, active leases, scoped cache, and freshness state with fixed sanitized diagnostics. Retained, fabricated, expired, disposed, removed, and non-current-account routes fail at the transport boundary; current occurrence-specific Max/image routing overrides stale caller metadata; explicit empty dynamic catalogs unregister models; and forced or silent TTL expiry revokes routes, scoped cache, and freshness state. Catalog publication/revocation aborts prior active and paused streams, multi-result resumes recheck synchronously before every write, local HTTP/2 stream bookkeeping is released before remote cleanup, and bounded disposal detaches never-settling cleanup tasks ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
- Made workflows and enabled-model/`--models` scope resolution await shared authenticated model discovery before validating explicit Cursor references. Qualified `cursor/<exact-id>` scope entries are byte-exact and fatal when unavailable during synchronous or deferred interactive startup—reasoning-looking suffixes are never stripped, and startup cannot continue prompting on a default/current model outside the requested scope. Bare (non-qualified) references are ordinary non-Cursor values resolved by the generic matchers and reserve no Cursor discovery; only an explicit lowercase `cursor/<id>` targets a Cursor route. Enabled-model scope recovery keeps a current in-scope selection or the saved default (only an omitted default is absent) before the first scoped entry. Explicit non-Cursor qualification remains unchanged; workflow caller cancellation detaches without cancelling other waiters, and non-Cursor workflows are not delayed ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
- Required an authenticated Cursor discovery capability before strict workflow preflight or stage model resolution can read the model registry, so list-only stale rows cannot create a Run, job, body, stage session, prompt, or transport. Interactive deferred startup now retries a settings-only exact Cursor default after extensions load—including a present blank route ID, which remains distinct from an omitted setting—while explicit and restored-session identity keep precedence and failed exact recovery still requires reselection ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
- Fenced delayed stored-discovery and execution Cursor credential lookups so an obsolete account, missing credential, or lookup error cannot overwrite or revoke a newer account catalog, cache, live execution authority, freshness state, or UI status, including after disposal; concurrent same-account executions still share one discovery ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
- Ordered Cursor cache saves and invalidations independently per account so delayed persistence cannot reload revoked state, erase a newer catalog, or recreate data after a later clear, and a later same-account save with an equal timestamp still replaces the earlier record; persistence failures remain best-effort with fixed sanitized diagnostics. Caller cancellation and provider disposal now detach pending credential resolver waits, leave late outcomes inert, and prevent transport invocation when cancellation wins immediately after authorization. An authorization failure or caller abort before a paused tool turn is resumed preserves that paused turn for a later authorized retry ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
- Preserved exact Cursor bytes in `--models` scopes and ordered main-chat fallback settings, so blank, whitespace, suffix-looking, and context-looking route IDs are never trimmed or reinterpreted; qualifier lookalikes cannot normalize into lowercase Cursor. Separate `--provider cursor --model ""` and eager scoped startup now retain an authoritative blank route ID, while omitted model settings keep their existing fallback behavior ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
