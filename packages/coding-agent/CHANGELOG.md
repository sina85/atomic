...
- Added feasibility-aware graduated protection to Verbatim Compaction so `contextCompact` returns a continuable result whenever the transcript can be made to fit the model's input budget, even when the strict reduction percentage is infeasible. A deterministic three-strategy fallback ladder (`meet_target` → `best_effort` → `evict_protected`) decouples the quality target from the liveness constraint: `meet_target` returns only when the quality target is met AND the result fits the liveness budget (`computeLivenessBudget(getEffectiveInputBudget(model), reserveTokens).tokens`), so a target-met-but-over-budget result escalates to the ladder instead of returning a non-continuable transcript; `best_effort` accepts a budget-fitting result without evicting protected entries — either when the target is provably infeasible and the planner's own result fits, or when the planner's result is over budget (or absent) but the **maximal feasible unprotected deletion set** (deterministically built via a validator-backed oldest-first greedy subset search) fits; `evict_protected` is reached only when the maximal feasible unprotected deletion set cannot fit the budget, force-evicting oldest protected task-bearing entries oldest-first, preserving the `preserve_recent` window and the most-recent task-bearing entry. New additive result fields `fitStrategy`, `evictedProtectedEntryIds`, and `achievedReductionPercent` record how compaction succeeded. Added a read-only `computeCompactionFeasibility` oracle (plus `computeLivenessBudget`, `buildEvictionTargets`, `buildMaxFeasibleDeletionRequest`) that reports the maximal achievable reduction, target feasibility, budget fit, and recommended strategy without mutating anything, reusing the same validator as the planner. When the unfiltered maximal deletable set fails validation as a whole (for example because of a tool-pairing invariant or the ≥1-task-bearing guard), the oracle performs a deterministic validator-backed feasible subset search (oldest-first greedy, revalidating the cumulative set each time) and reports the largest feasible reduction it can find instead of collapsing to zero.
- Added a prompt airlock that refuses a single user message whose estimated tokens exceed the liveness budget (`computeLivenessBudget(getEffectiveInputBudget(model), reserveTokens)`, clamped to ≥1) **before pre-prompt compaction** (right after model/auth validation, before `_checkCompaction`), so an oversized input is rejected without triggering compaction work or mutating session state. It throws a new additive `PromptExceedsBudgetError` (carrying `estimatedTokens`, `budgetTokens`, `modelId`), making the un-compactable-oversized-message pathology structurally impossible since user messages are protected and un-truncated. Two guards compose: the normal liveness-budget gate refuses messages whose estimated tokens exceed the clamped budget, and a targeted rejection refuses any non-empty prompt when the raw effective input budget minus the reserve is non-positive (`reserveTokens >= effectiveInputBudget`) — the clamped budget of 1 is a compaction-arithmetic safety floor, not a real input allowance, so even a single-token prompt like `"x"` is refused. The check uses the same `estimateTokens` heuristic and `computeLivenessBudget` as compaction so the input gate and the compaction budget agree. The airlock applies uniformly across all user-message entry points — `prompt` (non-streaming and streaming-queued), `steer`, `followUp`, and `sendUserMessage` (including queued delivery via `deliverAs`) — so streaming `steer`/`followUp` messages cannot bypass the budget gate.

### Changed

- `contextCompact` now throws only when no strategy can make the transcript fit the budget (genuinely irreducible) or on provider/abort errors — never merely because the quality percentage was missed. Sessions that still meet the target behave identically (`fitStrategy: "meet_target"`).
- `validateContextDeletionRequest` accepts an optional internal `{ evict?: boolean }` option that relaxes only the protection predicate (for the `evict_protected` strategy) while keeping every other invariant (≥1 task-bearing entry, tool-call/result pairing, latest-assistant-thinking integrity, recency). It is not added to the public deletion-tool schema exposed to the planner LLM.
