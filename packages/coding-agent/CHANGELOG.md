...
- Fixed transitive spend reconciliation to keep valid lower-bound usage while marking totals incomplete when session JSONL contains malformed nonblank lines, conservatively exclude fork-session usage when an inherited parent transcript is missing or malformed, avoid case/separator-sensitive Windows path double-counting for covered workflow-stage/subagent subtrees and aliases, and attribute async subagent and workflow-stage completion spend to the session that launched the run after session switches ([#1636](https://github.com/bastani-inc/atomic/issues/1636)).
