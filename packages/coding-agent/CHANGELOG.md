...
- Fixed cross-session `/workflow resume` so exact `--session` restoration resumes the authoritative durable root instead of an ended snapshot, and a restored `running` snapshot after a hard exit (Ctrl-C/crash before the graceful quit-to-paused flush) is no longer wrongly refused as "already running in this session" — active-run refusal now consults the real execution lease, not the reconstructed history entry; per-dispatch heartbeat-backed file leases and PostgreSQL advisory DBOS leases prevent duplicate execution across processes and hosts while recovering crashes safely (a dropped advisory-lock connection stops pinning ownership, a reused PID is reclaimed via a locale/timezone-independent process-generation identity or, when that identity is unconfirmable, once its heartbeat goes stale, transient heartbeat hiccups are retried, and a completed workflow no longer leaks its heartbeat interval); foreign-host file leases are conservatively pinned to prevent cross-host double-dispatch (cross-host coordination uses the connection-fenced DBOS/PostgreSQL backend); paused metadata retains ownership until execution stops; a cancellation during the resume publication flush aborts before workflow code runs; a failed rapid re-resume no longer double-releases or rolls back over a newer owner's lease/status; file-lock handoff is atomic, process-generation aware, and compares the reclaimed lock against the exact owner snapshot it checked; durable reads revalidate against disk so an externally pruned completed root cannot be resurrected from a stale cache; stale picker rows and DBOS metadata are revalidated (targeted remote-ownership refresh clears a stale external-owner marker before refusing resume); durable setup is awaited before success; Windows-illegal workflow IDs are encoded; changed definitions are refused; and resumable roots are ordered deterministically.
