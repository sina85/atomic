...
The footer's dollar amount and token/cache badges (`↑`, `↓`, `R`, `W`) are transitive: they include the current session plus its subagent, nested subagent, and workflow-stage descendants, and they climb live as those descendants spend — workflow stages report incrementally on every turn, not only when a stage finishes, including when `/workflow` is the first thing typed in a fresh chat. Replayed workflow stages restore persisted spend to the launching session without rerunning provider work. The context-window percentage stays current-session only, since it measures this session's prompt against its own model window. A `~` prefix on the dollar amount marks a lower-bound total while async descendants are unsettled, a complete session file lags newer terminal scalar usage, or session-file reconciliation encountered unreadable lines. When fork ancestry cannot be trusted, file-derived inherited usage is omitted while direct terminal scalar usage remains as a conservative floor. Reconciliation never reduces known settled usage when stale live/durable aliases arrive. On subscription/OAuth models the dollar figure can stay `$0.000` when the provider reports no per-token price, while the token badges still climb, so delegated spend remains visible. Use `/cost` to reconcile descendant session files and show a self, per-descendant, and total breakdown.
