...
### Graduated Protection: Fitting the Budget

The deletion target (`1 - compression_ratio`, default 50%) is a **quality target**, not a hard gate. The real constraint compaction must satisfy is **liveness**: the post-compaction transcript must fit the liveness budget (`computeLivenessBudget(getEffectiveInputBudget(model), reserveTokens)`, clamped to ≥1). `contextCompact` uses a deterministic three-strategy fallback ladder so a session stays continuable even when the strict percentage is infeasible:

1. **`meet_target`** — the planner met the quality target under full protection **and** the result fits the liveness budget. If the target is met but the result still overflows the budget, `meet_target` is skipped and the ladder escalates.
2. **`best_effort`** — accepted when a budget-fitting result exists **without** evicting protected entries. Two sub-cases: (a) the quality target is **provably infeasible** and the planner's own validated result already fits the budget; (b) the planner's result is over budget (or absent) but the **maximal feasible unprotected deletion set** — deterministically built via a validator-backed oldest-first greedy subset search — fits the budget. The result is recorded with `fitStrategy: "best_effort"`. A merely under-performing planner whose own result does not fit and for which no maximal feasible unprotected set fits is **not** accepted here — it escalates to eviction or errors so a lazy planner is never silently rewarded.
3. **`evict_protected`** — only reached when the maximal feasible **unprotected** deletion set still cannot fit the budget (protected mass alone overflows). Atomic deterministically force-evicts the **oldest** protected task-bearing entries (`user`/`custom`/`branchSummary`), oldest-first, stopping the instant the result fits. Eviction never touches the `preserve_recent` window and never evicts the most-recent task-bearing entry; the validator's ≥1-task-bearing guard is the final backstop.

The strategy used is recorded on the result as `fitStrategy` (`"meet_target" | "best_effort" | "evict_protected"`), with `evictedProtectedEntryIds` listing any force-evicted entry ids and `achievedReductionPercent` mirroring `stats.percentReduction`. `contextCompact` throws only when no strategy can make the transcript fit the budget (genuinely irreducible), or on provider/abort errors — never merely because the quality percentage was missed.

A read-only oracle, `computeCompactionFeasibility`, reports the maximal achievable verbatim reduction, whether the target is feasible, whether the result fits the budget, and the recommended strategy — without mutating anything. It reuses the same validator as the planner so the oracle and the planner agree on what is actually deletable. When the unfiltered maximal deletable set fails validation as a whole (for example because of a tool-pairing invariant or the ≥1-task-bearing guard), the oracle performs a deterministic validator-backed feasible subset search (oldest-first greedy, revalidating the cumulative set each time) and reports the largest feasible reduction it can find instead of collapsing to zero.

### Prompt Airlock: Refusing Un-Fittable Input

A single user message that exceeds the model's input budget can never be made to fit by compaction (user messages are protected and un-truncated). To make that pathology structurally impossible, the airlock refuses a user message whose estimated tokens exceed the liveness budget (`computeLivenessBudget(getEffectiveInputBudget(model), reserveTokens)`, clamped to ≥1) **before pre-prompt compaction** — it runs right after model/auth validation and before `_checkCompaction`, so an oversized input is rejected without triggering compaction work or mutating session state. It throws a `PromptExceedsBudgetError` that carries `estimatedTokens`, `budgetTokens`, and `modelId`; the error message tells the user to shorten/split the input, remove large attachments, or switch to a larger-context model. Two guards compose: the normal liveness-budget gate refuses messages whose estimated tokens exceed the clamped budget, and a **targeted rejection** refuses any non-empty prompt when the raw effective input budget minus the reserve is non-positive (`reserveTokens >= effectiveInputBudget`) — the clamped budget of 1 is a compaction-arithmetic safety floor, not a real input allowance, so even a single-token prompt like `"x"` is refused in that configuration. The check uses the same `estimateTokens` heuristic and `computeLivenessBudget` as compaction so the input gate and the compaction budget agree. The airlock applies uniformly across every user-message entry point: `prompt` (non-streaming and streaming-queued), `steer`, `followUp`, and `sendUserMessage` (including queued delivery via `deliverAs: "steer" | "followUp"`).
