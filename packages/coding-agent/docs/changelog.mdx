...
- **Incomplete totals are explicit.** The footer, `/context`, and `/cost` use a `~` lower-bound marker while descendant spend is unsettled or session-file reconciliation encounters malformed lines. The context-window percentage still describes the active session; use `/cost` for the self/descendant/total breakdown and durable reconciliation.
