...
- **Live workflows update the launcher.** Background `/workflow` runs and subagent-heavy workflows such as `deep-research-codebase` stream unsettled usage rollups back to the launching chat while child agents are still running; async completions stay attributed to the session that launched them after session switches.
- **Incomplete totals are explicit and monotonic.** The footer, `/context`, and `/cost` use a `~` lower-bound marker while descendant spend is unsettled or session-file reconciliation encounters malformed lines. Settled descendant totals are not downgraded by delayed interim updates, workflow stages reconcile descendants before persisting terminal usage, and stale session walks cannot overwrite newer completeness. The context-window percentage still describes the active session; use `/cost` for the self/descendant/total breakdown and durable reconciliation.
