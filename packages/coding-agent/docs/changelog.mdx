...
- **Model controls are current.** Atomic now accepts the `max` thinking level wherever the active model advertises it, and `models.json` overrides apply to matching extension-registered models as well as built-ins. When both legacy `.pi` and primary `.atomic` files exist, disjoint override model IDs survive while an exact primary entry replaces the complete legacy entry with source-correct header precedence. See [Models](/models).
...
- **Safer session and tool behavior.** Invalid explicit bash timeouts fail instead of being clamped; missing exact session IDs warn before creation; auth writes surface persistence failures; Windows context traversal terminates at drive roots; and duplicate fork selections are ignored. See [Tools](/tools) and [Usage](/usage).
- **More reliable streaming and binaries.** Visible custom messages stay before the live assistant row, standalone Linux clipboard reads can fall back to xclip with correctly packaged native bindings, and caller-relative `TMPDIR` paths work from external directories for both normal and `--skip-deps` builds.
