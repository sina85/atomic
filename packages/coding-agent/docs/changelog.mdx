...
- **Model controls are current.** Atomic now accepts the `max` thinking level wherever the active model advertises it, and `models.json` overrides apply to matching extension-registered models as well as built-ins. See [Models](/models).
...
- **More reliable streaming and binaries.** Visible custom messages stay before the live assistant row, and standalone Linux clipboard reads can fall back to xclip with correctly packaged native bindings.
