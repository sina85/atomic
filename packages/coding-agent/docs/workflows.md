...
Status surfaces include a compact `Loop:` rail derived from the existing run snapshot rather than new workflow metadata. `/workflow status` shows a `loop` row under active multi-stage, bounded-loop, or builtin workflow runs; one-shot single-stage rows and terminal run cards omit it so completed work does not look pending. `/workflow status <run-id>` includes a matching `LOOP` section for live runs where the rail applies, and the graph overlay statusline fits `Loop: ...` after the `[ GRAPH ]` pill while preserving keyboard hints and using the same expanded child stages shown in the graph. Expanded child workflow stages participate in that graph rail, builtin non-positive loop limits display the same positive defaults their runners use, generic bounded loops can read matching `*_completed` result counters, and ordinary numeric stage labels such as `oauth-2` stay literal unless Atomic sees a repeated counted family. Very narrow graph overlays hide the loop rail before truncating the key hints away. Use the detail command when a narrow terminal compresses or hides the rail.
