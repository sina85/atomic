...
When Atomic starts with an existing session or resumes one, it shows an informational notice for root workflows whose latest session checkpoint has the same resume eligibility as the durable catalog: `running`/`paused` entries need checkpoint progress or a pending prompt, while `failed`/`blocked` entries are eligible unless they explicitly set `resumable: false`. Completed, cancelled, zero-progress running/paused, nested-child, and explicitly non-resumable failed/blocked entries are omitted. The notice provides `/workflow resume <id>` commands; the command still validates the authoritative durable backend and current workflow registry before dispatch, so stale session-cache metadata cannot resurrect terminal or missing durable state.
