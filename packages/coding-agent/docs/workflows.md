...
Status surfaces include a compact `Loop:` rail derived from the existing run snapshot rather than new workflow metadata. `/workflow status` shows a `loop` row under each run's chain progress, `/workflow status <run-id>` includes a `LOOP` section after artifacts, and the graph overlay statusline fits `Loop: ...` after the `[ GRAPH ]` pill while preserving keyboard hints. Use the detail command when a narrow terminal compresses the rail.
