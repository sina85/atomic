...
Cursor is a deliberate exception to this fallback chain. Before validation, workflows await the shared authenticated discovery lifecycle for explicit Cursor strings, Cursor `Model` objects, and rejection-only bare historical direct IDs. Named `workflow({ action: "run" })` launches complete this validation before creating a Run, store entry, or stage session; concurrent launches share discovery, cancellation creates no Run, and non-Cursor named launches do not wait. A matching exact route is replaced with the live row. A stale or unavailable reference is a configuration/reselection failure, so it does not advance to configured fallbacks, the current model, another provider, or another Cursor route. If a bare historical value such as `composer-2` is currently returned by Cursor, that exact row wins; otherwise the tombstone rejects cross-provider resolution and users must explicitly qualify any intended non-Cursor collision. Generic non-Cursor provider-qualified pass-through and fallback behavior remains unchanged, and builtins do not hard-code account-specific Cursor routes.
