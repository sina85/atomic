...
Cursor is a deliberate exception to this fallback chain. Before validation, workflows await the shared authenticated discovery lifecycle only for explicit strings with the literal lowercase `cursor/` prefix and `Model` objects whose provider identity is exactly lowercase `cursor`; bare (non-qualified) references are ordinary non-Cursor values and reserve no Cursor discovery. Named `workflow({ action: "run" })` launches complete this validation before creating a Run, store entry, or stage session; concurrent launches share discovery, cancellation creates no Run, and non-Cursor named launches do not wait. A selected Cursor model object resolves to the live catalog occurrence — chosen by the object's private per-ID ordinal when it indexes an existing occurrence, otherwise the first occurrence — so its executable api, base URL, and routing come only from the authenticated catalog; the caller's object is never executed. A stale or unavailable reference is a configuration/reselection failure, so it does not advance to configured fallbacks, the current model, another provider, or another Cursor route. Case- or whitespace-variant provider identities remain ordinary non-Cursor values and are never normalized into Cursor discovery or errors. A bare value such as `composer-2` is resolved by the ordinary non-Cursor matchers; to target a Cursor route, qualify it explicitly as `cursor/<id>`. Generic non-Cursor provider-qualified pass-through and fallback behavior remains unchanged, and builtins do not hard-code account-specific Cursor routes.
