...
- **Crash-safe execution ownership**: Default file-backed durability stores each root workflow in its own atomically lock-protected JSON file under `~/.atomic/workflow-durable`; workflow IDs are percent-encoded so filenames remain valid on Windows. File execution leases record host, PID, process-generation identity, and a per-dispatch token while refreshing a heartbeat. Same-host PID/generation liveness prevents debugger stops or event-loop stalls from creating split-brain; dead same-host owners are reclaimed immediately, migrated-host stale records are heartbeat-reclaimed, and only the matching dispatch token can release ownership. DBOS uses PostgreSQL session advisory locks instead of host-local files, so ownership coordinates across hosts and is released automatically if the owning database connection/process crashes. Both backends hide active owners, refuse targeted duplicate resume, retain ownership through paused metadata until the executor stops, and flush durable registration before reporting resume success. State-lock acquire/release uses atomic rename to prevent handoff races. Completed/cancelled/non-resumable files are pruned, and file writes use `0700` directories and `0600` files where supported. Durable replay identities use SHA-256 over canonical JSON.
...
**Resume after editing a workflow.** New durable runs persist a hash of the workflow name, schemas, input bindings, and executable `run` definition. If that definition changes between quit/crash and resume, Atomic refuses before stage execution rather than interpreting old checkpoints with new code; restore the original definition to finish the run, or kill it and start a new run. Legacy durable records created before definition hashes were introduced retain their previous best-effort checkpoint matching behavior.
...
| **Process crash** | Workflow remains `running` in durable state, but its dead same-host execution lease is reclaimed. On next session start, it appears in the resumable selector. Resume re-executes from the last completed checkpoint. A still-live owner stays hidden and targeted resume is refused, preventing cross-process duplicate dispatch. |
