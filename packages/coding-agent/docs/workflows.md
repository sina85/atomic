...
Status surfaces include a compact rail derived from the existing run snapshot rather than new workflow metadata. Bounded workflows show a `Loop:` rail/`loop` row with remaining-round wording, while active multi-stage workflows without a bounded-loop hint use neutral `Phases:`/`phases` labels so a one-pass pipeline does not imply future repetition. One-shot single-stage rows and terminal run cards omit the rail so completed work does not look pending. `/workflow status <run-id>` includes a matching `LOOP` or `PHASES` section for live runs where the rail applies, and the graph overlay statusline fits the same prefixed rail after the `[ GRAPH ]` pill while preserving keyboard hints and using the expanded child stages shown in the graph. Expanded child workflow stages participate in that graph rail, builtin non-positive loop limits display the same positive defaults their runners use, generic bounded loops can read matching `*_completed` result counters, and ordinary numeric stage labels such as `oauth-2` stay literal unless Atomic sees a repeated counted family. Very narrow graph overlays hide the rail before truncating the key hints away. Use the detail command when a narrow terminal compresses or hides the rail.
