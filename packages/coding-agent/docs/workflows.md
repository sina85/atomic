...
/workflow resume <run-id> [stage] msg  # resume paused/failed, or open completed durable runs
...
The default file backend retains completed workflow files with checkpoint data so `/workflow resume` can reopen them for inspection without re-running work. Running, paused (quit), recoverably failed, and successful completed workflows with checkpoint progress are retained indefinitely by design; cancelled and explicitly non-resumable failed/blocked workflows are removed. Use `/workflow kill <id>` to mark a durable workflow cancelled/non-resumable and remove its file-backed state, or delete the corresponding `workflow-<encoded-id>.json` file under `~/.atomic/workflow-durable` if you intentionally want to discard abandoned durable state outside Atomic.
...
The `/workflow resume` command mirrors `/resume` ergonomics and now uses the same Atomic session-selector tree chrome as `/resume` for its interactive picker. With no id, interactive sessions show one searchable/threaded selector that lists live paused/failed runs, resumable durable runs, and successful completed durable workflows together. Selecting a resumable durable workflow re-dispatches with the original workflow id so completed checkpoints replay without side effects; selecting a completed durable workflow opens a read-only completed run snapshot in the workflow detail/chat overlay instead of re-running stages, tools, tasks, or prompts. Completed rows use `✓ completed` labeling and are hidden when the durable backend/checkpoints or referenced session files are unavailable.

```text
/workflow resume                          # Show resumable and completed workflows
/workflow resume <workflow-id-or-prefix>  # Resume resumable runs or open completed runs
```

The selector displays workflow rows through the same `/resume` tree UI and lists only **inactive, safe workflow targets**: paused (quit) or recoverably-failed durable runs with checkpoint progress, successful completed durable runs with authoritative checkpoint state, plus any live paused/failed runs. Actively-running workflows are hidden from the selector — resuming one that is already executing (in this or another session) would double-dispatch, so `/workflow resume <running-id>` is refused with an intuitive error pointing at `/workflow connect` (to attach) or `/workflow kill` (to clear a stuck run). Quitting the CLI/panel flips the durable handle from `running` to `paused`, which is what makes a workflow re-enter the resumable set; a fresh dispatch or durable resume flips it back to `running`. Running/paused durable rows appear only after the workflow has at least one durable checkpoint or pending checkpointable prompt, so a just-started workflow with no resumable state does not clutter the picker. Eligible LM stages also write their Atomic/Pi session metadata as soon as the stage session opens; on durable resume, Atomic reopens that exact session file and sends `Continue` instead of replaying the original workflow prompt, so repeated quit/resume cycles append to the active LM chat rather than duplicating prompts or emptying prior chats. Successful completion marks the durable handle `completed`; completed entries stay discoverable for inspection but use an explicit open path that never calls the durable resume/replay dispatcher. Quitting the CLI is treated as a resumable process boundary rather than explicit workflow cancellation, so durable-progress workflows remain available to `/workflow resume`; use `/workflow kill` when you want a workflow removed as cancelled/non-resumable. If no resumable or completed workflows remain after filtering, interactive `/workflow resume` still opens the same empty selector rather than printing an error, matching `/resume` behavior. In fresh sessions, Atomic scans workflow-specific `workflow.durable.checkpoint` JSONL history for discovery, but the durable backend remains authoritative: cache rows without backend checkpoint state are treated as old incompatible metadata and are hidden instead of prompting a stale resume failure. File-backed durability is enabled by default under `~/.atomic/workflow-durable`, so `/workflow resume` can discover cross-session workflows without any setup.
