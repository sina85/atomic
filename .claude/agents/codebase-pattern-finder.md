...
Do NOT use `codegraph_explore` or `codegraph_context` — these return large source dumps that fill context. Use `codegraph_callers`/`codegraph_callees` for targeted usage discovery instead.
